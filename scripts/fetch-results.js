...
async function main() {
  ...
  for (const m of matches) {
    ...
    if (koRound) {
      ...
      // For R16+ fixtures the home/away codes are TBD at draw time so koTeamPairMap has no
      // entry. Fall back to resolving both sides of every same-round fixture dynamically —
      // this works because R16+ matches are processed after all their source matches have
      // already been recorded in base.knockoutStage.
      if (!fixtureId && homeCode && awayCode && koRound !== "R32") {
        for (const kofx of (fixtures.knockoutFixtures || []).filter(f => f.round === koRound)) {
          ...
          if ((h === homeCode && a === awayCode) || (h === awayCode && a === homeCode)) {
            fixtureId = kofx.id;
            console.log(`  ~ KO matched by resolved team codes (timestamp mismatch): ${m.homeTeam.name} v ${m.awayTeam.name} ${m.utcDate}`);
            break;
          }
        }
      }
  ...
  const merged = mergeResults(base, overrides, fixtures);
